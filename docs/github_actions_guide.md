...
- workflow 側では Gemini の model / fallback / request limit も `infra/.env` に明示しています
...
- stale watchlist は `main patrol` で使わない
- report / 通知で `watchlist状態`、`watchlist候補変化`、`AI状態` を追える
- report で `Runtime Config` と `AI request config` も追える
- 通知で `AIリクエスト設定変化` も追える
